...
5. ML prediction corrections tracked in `internal/service/transaction.go` (`UserCorrectedPayee`, `UserCorrectedCategory`, etc.) The review queue (`/api/predictions/review`) surfaces cipher predictions least-confident-first. Both a **correction** and a **confirmation** teach the pipeline: `transactionService.Update` queues payee-rule learning when payee/category changed (also marking the prediction user-corrected) *or* when the row goes `UNAPPROVED → APPROVED` with the mapping untouched (`predictionConfirmed`), and `transactionService.UpdateStatus` — what the Approve button hits (`PATCH /api/transactions/:id/status`) — learns on approval. Learning is skipped for rows with no cipher prediction, since a manual transaction has no raw bank text to match future emails on. The review queue's **accept** action learns too (`predictionReviewService.Review` → `transactionService.LearnFromTransaction`). `learnTransactionMappingAsync` wraps a synchronous `learnTransactionMapping` in a goroutine and is best-effort — nothing upstream sees its failures — so the outcome is stamped on the prediction instead: `cipher_predictions.learned_at` is set on success and `learn_error` records why the last attempt failed. That makes a broken learning step visible and lets past predictions be replayed: `GET /api/predictions/learning` returns `{total, learned, pending, failed, skipped}` (pending counts only rows a backfill can actually process; skipped are rows with no payee/category/raw bank text, so `total = learned + pending + skipped`), and `POST /api/predictions/learning/backfill` (`{limit, retryFailed}`, default 25 / max 100) re-runs learning over unlearned rows oldest-first. The backfill **runs in the background** and answers `202` with `{started, running, queued, remaining}` — a page of items costs two LLM round-trips each and would blow the proxy's request timeout (Cloudflare 524), so progress is durable rather than streamed: each item stamps `learned_at`/`learn_error` as it finishes and callers poll the status endpoint, whose `running` flag distinguishes "still working" from "nothing left". Runs are single-flighted per process (`transactionService.learningBackfillRunning`) so a double click cannot pay for the same page twice; a run is capped at 1h and each item at 5m. Items run sequentially — a local model serves them one at a time anyway.
...
7. Receipts/documents: `transaction_documents` rows + bodies in Postgres (`document_blobs`, migration `00019`). `internal/storage` has two `Store` implementations sharing one key scheme: `postgres.go` (default; `bytea`, `SET STORAGE EXTERNAL` so already-compressed PDFs/JPEGs skip a pointless compression pass) and `local.go` (disk under `UPLOADS_DIR`, opt in with `DOCUMENT_STORAGE=local`). Keeping bodies in the database means one `pg_dump` backs everything up and a body cannot outlive its row; the cost is database size, which is why the 10MB/40MB upload caps matter. Keys are `<budgetId>/<transactionId>/<Payee>_<YYYYMMDD>-<n><ext>` (e.g. `McDonalds_20260819-1.pdf`) — the index is always present so adding a second document never renames the first, and the transaction segment stops two same-payee same-day transactions colliding. `POST/GET /api/transactions/:id/documents` (multipart field `file`, 10MB cap, content-sniffed mime whitelist: jpeg/png/webp/heic/pdf), `GET /api/documents/:docId/content` (authenticated streaming; frontends fetch with auth headers → blob/object URL since `<img src>` can't carry Authorization), `DELETE /api/documents/:docId` (soft-delete + file removal). Multi-page scanning: the Android app uses ML Kit's document scanner (`react-native-document-scanner-plugin` — edge detection, perspective correction, enhance filters; needs a prebuilt dev client, not Expo Go) and posts the pages to `POST /api/transactions/:id/documents/scan` (repeated `pages` parts, ≤20 pages, 40MB total), which merges them into one A4 PDF via `internal/service/documentScan.go` and stores it as a single document. The React panel exposes the same endpoint as "Combine images to PDF". Beyond the per-transaction panel, `GET /api/documents?search=&type=image|pdf&startDate=&endDate=&limit=&offset=` serves a **budget-wide document library** (`transactionDocumentRepo.Search`): each row is a `model.DocumentListItem` — the document plus the payee/date/amount/account/category of its transaction, so a gallery tile needs no follow-up request. The join to `transactions` is INNER, dates filter on the *transaction* date (not the upload), and paging is offset-based (`limit`+1 probe sets `hasMore`; 50 default, 100 max) rather than the cursor scheme transactions use, since the grid is only ever scrolled from the newest end. Web surfaces it at Settings → Documents (`react-frontend/src/features/documents/`), with tiles linking back to `/transactions/:id`. Android surfaces it twice: a "Documents" row under Settings → Library opening a full-screen `DocumentsScreen` on the root stack, and a "Recent receipts" strip on the Dashboard that hides itself when the budget has no documents. Mobile substitutes period chips (any time / this month / last 3 months / this year) for the web's date inputs since the app has no native date picker. **Viewing and downloading are separate operations on Android** (`android-frontend/src/features/documents/documentFile.ts`): `ensureDocumentCached` pulls the body into the app's private cache directory — evicted under storage pressure, absent from the gallery and Downloads — and `DocumentViewer` renders from there, so opening a receipt leaves no copy on the device; `saveDocumentToDevice` is the explicit action, copying the same bytes to a user-picked folder via the Storage Access Framework (the only way to write outside the sandbox without a legacy storage permission). Images render through `<Image>` with pinch-zoom, PDFs through `react-native-pdf`, which rasterises from the cached path rather than the API URL so the native layer never needs auth headers. Both actions appear on the library's detail sheet and on a transaction's receipt strip. `react-native-pdf` and its `react-native-blob-util` peer are native modules: rebuild the dev client after pulling them in.
8. Pipeline observability: the email workflows report progress via `StartPipelineRun`/`ReportPipelineStatus` activities (hosted by the go-pennywise-api worker) into `pipeline_runs` + `pipeline_run_events`; the fetch step emits one `fetch_emails` event per email carrying `from`/`subject`/`snippet` (captured from Gmail headers + snippet in `FetchEmailData`) so the Activity page can show sender/preview/message id before extraction runs, the run row stores the triggering `gmail_history_id`, and the per-email loop emits one timeline event per email (parse/predict succeeded/skipped/failed, keyed by `messageId`) and each run change is broadcast budget-wide as the `pennywise::pipeline::update` websocket event (`sharedModel.EventPipelineUpdate`). Every LLM/embedding round-trip is recorded as it happens (`cipher/internal/llmusage` collector, filled by `chatWithFallback`/`embedWithFallback` with provider, model, in/out tokens, duration and whether the attempt failed over), returned on `ParseEmailResult`/`PredictEmailResult.llmCalls`, attached to that email's timeline event and accumulated onto the run as `llm_calls`/`input_tokens`/`output_tokens` plus the per-model `llm_usage` JSONB breakdown — retried emails add to the totals, since they really spend the tokens twice. The React `/activity` page lists runs, shows per-email extraction/prediction detail with the model and token cost of each step and a per-model usage table per run, and can retry parked workflows through `POST /api/pipeline/runs/:id/retry` (signals Temporal directly, choosing `retry-email-parse` vs `retry-predict` from the run's `current_step`). Reporting is best-effort — status write failures never fail the pipeline — and is gated by `workflow.GetVersion` marker "pipeline-observability" so pre-rollout in-flight workflows replay unchanged.
...
| Category resolution (exact + trigram fuzzy) | `backend/cipher/internal/service/prediction.go` (`llmFallback`, `resolveCategoryFuzzy`), `backend/shared/db/category.go` (`FindClosestSimplified`) |
...
| Learning backfill (`/api/predictions/learning[/backfill]`) | `backend/go-pennywise-api/internal/service/transaction.go` (`learnTransactionMapping`, `BackfillLearning`), `backend/shared/db/cipherPrediction.go` |
