...
func NewMonthlyBudgetRepository(pool *pgxpool.Pool) MonthlyBudgetRepository {
	return &monthlyBudgetRepo{BaseRepository: NewBaseRepository(pool)}
}
...
func (r *monthlyBudgetRepo) UpdateCarryoverByCatIdAndMonth(
	...
	cmdTag, err := r.Executor(tx).Exec(ctx, `
		UPDATE monthly_budgets
		SET carryover_balance = carryover_balance + $1
		WHERE budget_id = $2 AND TO_DATE(month, 'YYYY-MM') >= TO_DATE($3, 'YYYY-MM') AND category_id = $4
		`, amount, budgetId, month, categoryId,
	)
	if err != nil {
