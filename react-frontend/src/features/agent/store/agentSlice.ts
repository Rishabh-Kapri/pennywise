...
function mergeMessageParts(existingParts: MessagePart[] = [], newParts: MessagePart[] = []) {
  ...
  for (const part of newParts) {
    const existingIndex = part.id
      ? mergedParts.findIndex((existingPart) => existingPart.id === part.id)
      : -1;
    if (existingIndex >= 0) {
...
function chatMessagesFromConversationMessages(messages: ConversationMessage[]) {
  ...
  return Array.from(groupedMessages.values())
    .filter((chatMessage) => chatMessage.text.trim() !== '' || (chatMessage.parts?.length ?? 0) > 0);
}
...
function moveConversationToFront(state: AgentState, conversationId: string) {
  state.chatHistoryOrder = [
    conversationId,
    ...state.chatHistoryOrder.filter((id) => id !== conversationId),
  ];
}
...
export const createAgentRun = createAsyncThunk<AgentRun, CreateAgentRunArgs>(
  'agent/createRun',
  async (args, { getState }) => {
    const state = getState() as RootState;
    const conversationId = args.conversationId ?? state.agent.currentConversationId ?? undefined;
    const selectedModel =
      AGENT_MODEL_OPTIONS.find((option) => option.key === state.agent.selectedModelKey) ??
    ...
    return await apiClient.post<AgentRun>('agent/runs', payload);
  },
);

export const listAgentConversations = createAsyncThunk<AgentConversation[]>(
  'agent/listConversations',
  async () => {
    return await apiClient.get<AgentConversation[]>('agent/conversations');
  },
);

export const fetchAgentConversationMessages = createAsyncThunk<
...
export const selectAgentCreateRunLoading = (state: RootState) =>
  state.agent.createRunLoading;
export const selectAgentError = (state: RootState) => state.agent.error;
export const selectCurrentAgentConversationId = (state: RootState) =>
  state.agent.currentConversationId;
export const selectCurrentAgentStreamId = (state: RootState) => state.agent.currentStreamId;
export const selectSelectedAgentModelKey = (state: RootState) =>
  state.agent.selectedModelKey;
