...
export const fetchPipelineRuns = createAsyncThunk<PipelineRun[]>(
  'pipeline/fetchPipelineRuns',
  async () => {
    return await apiClient.get<PipelineRun[]>(
      `pipeline/runs?limit=${RUNS_FETCH_LIMIT}`,
    );
  },
);

export const fetchPipelineRunDetail = createAsyncThunk<
  PipelineRunDetail,
  string
>('pipeline/fetchPipelineRunDetail', async (runId) => {
  return await apiClient.get<PipelineRunDetail>(`pipeline/runs/${runId}`);
});

export const retryPipelineRun = createAsyncThunk<
  { status: string; run: PipelineRun },
  string
>('pipeline/retryPipelineRun', async (runId) => {
  return await apiClient.post<{ status: string; run: PipelineRun }>(
...
export const selectPipelineRuns = (state: RootState) => state.pipeline.runs;
export const selectActivePipelineRunCount = (state: RootState) =>
  state.pipeline.runs.filter(
